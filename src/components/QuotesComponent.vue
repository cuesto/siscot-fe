                        ...
                        <v-layout wrap>
                            ...
                            <v-flex xs12 md3>
                                <v-menu v-model="menuDocDate" :close-on-content-click="false" :nudge-right="40" transition="scale-transition" offset-y min-width="290px">
                                    <template v-slot:activator="{ on }">
                                        <v-text-field v-model="invoiceDraftModel.docDate" label="Fecha de documento" prepend-icon="event" readonly :rules="[rules.required]" v-on="on"></v-text-field>
                                    </template>
                                    <v-date-picker v-model="invoiceDraftModel.docDate" @input="menuDocDate = false"></v-date-picker>
                                </v-menu>
                            </v-flex>
                            <v-flex xs12 md12>
                                <v-data-table :headers="headers" :search="search" :items="invoiceDraftModel.invoiceDraftDetail" sort-by="itemcode" class="elevation-1">
                                    <template v-slot:top>
                            ...
                            <v-container>
                                <v-row no-gutters>
                                    <v-col cols="12" sm="6">
                                        <v-container fluid>
                                            <v-textarea counter solo label="Comentarios" v-model="invoiceDraftModel.comments"></v-textarea>
                                        </v-container>
                                    </v-col>
                                    <v-col cols="12" sm="2">
                                    ...
                                    <v-col cols="12" sm="4">
                                        <v-card class="pa-2" outlined tile>
                                            <v-text-field disabled label="Subtotal" v-model="invoiceDraftModel.subTotal"></v-text-field>
                                            <v-text-field disabled label="Impuestos" v-model="invoiceDraftModel.tax"></v-text-field>
                                            <v-text-field disabled label="Total" v-model="invoiceDraftModel.total"></v-text-field>
                                        </v-card>
                                    </v-col>
                                </v-row>
                            </v-container>
...
export default {
    ...
    methods: {
        ...
        addItem() {
            ...
            itemToAdd.invoiceDraftDetailKey =
        ...
        calculateTotals() {
            this.invoiceDraftModel.subTotal =
                this.invoiceDraftModel.invoiceDraftDetail.reduce(
                    (accum, item) => accum + item.total,
                    0
                );

            this.invoiceDraftModel.tax =
                this.invoiceDraftModel.invoiceDraftDetail.reduce(
