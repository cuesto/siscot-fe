            ...
            <v-card title="Cotización" elevation="0" :disabled="disabled">
                <v-form ref="form">
                    <v-container py-0>
                        <v-layout wrap>
                            <v-flex xs12 md5>
                                <v-autocomplete v-model="quotationModel.CardCode" :items="businessPartners" :rules="[rules.required]" color="blue-grey lighten-2" label="Socio de Negocios" item-text="displayAutoComplete" item-value="CardCode" hint="Código - Nombre - RNC/Cédula" :loading="loadingBP"></v-autocomplete>
                                    ...
                                    <v-form ref="formDialogBP">
                                        <v-card>
                                                ...
                                                <v-container>
                                                    <v-row>
                                                        <v-col cols="7" sm="6" md="6">
                                                            <v-text-field label="Nombre" :rules="[rules.required]" v-model="bpModel.CardName"></v-text-field>
                                                        </v-col>
                                                        <v-col cols="5" sm="6" md="6">
                                                            <v-select :loading="loadingNCFTypes" undefined v-model="bpModel.U_Tipo_NCF" :items="ncfTypes" item-text="Name" item-value="Code" :rules="[rules.required]" color="blue-grey lighten-2" label="Tipo NCF"></v-select>
                                                        </v-col>
                                                    </v-row>
                                                    <v-row>
                                                        <v-col cols="6" sm="6" md="6">
                                                            <v-text-field label="Email" :rules="[rules.required, rules.email]" v-model="bpModel.EmailAddress"></v-text-field>
                                                        </v-col>
                                                        <v-col cols="6" sm="6" md="6">
                                                            <v-text-field label="Celular" :rules="[rules.required]" v-model="bpModel.Phone1"></v-text-field>
                                                        </v-col>
                                                    </v-row>
                                                    <v-row>
                                                        <v-col cols="6" sm="6" md="6">
                                                            <v-select undefined v-model="bpModel.U_Tipo_ID" :items="idTypes" item-text="Name" item-value="Code" :rules="[rules.required]" color="blue-grey lighten-2" label="Tipo ID"></v-select>
                                                        </v-col>
                                                        <v-col cols="6" sm="6" md="6">
                                                            <v-text-field label="RNC/Cédula" :rules="[rules.required]" v-model="bpModel.FederalTaxID"></v-text-field>
                                                        </v-col>
                                                    </v-row>
                                                    <v-row>
                                                        <v-col cols="6" sm="6" md="6">
                                                            <v-select undefined v-model="bpModel.Indicator" :items="indicators" item-text="Name" item-value="Code" :rules="[rules.required]" color="blue-grey lighten-2" label="Tienda"></v-select>
                                                        </v-col>
                                                        <v-col cols="6" sm="6" md="6">
                                                            <v-select undefined v-model="bpModel.SalesPersonCode" :items="salesPersons" item-text="SlpName" item-value="SlpCode" :rules="[rules.required]" color="blue-grey lighten-2" label="Vendedor"></v-select>
                                                        </v-col>
                                                    </v-row>
                                                    <v-divider></v-divider>
                                                    <v-row>
                                                        <v-col cols="12" sm="12" md="12">
                                                            <v-text-field label="Dirección" :rules="[rules.required]" v-model="bpModel.AddressName"></v-text-field>
                                                        </v-col>
                                                    </v-row>
                                                    <v-row>
                                                        <v-col cols="12" sm="12" md="12">
                                                            <v-text-field label="Calle" :rules="[rules.required]" v-model="bpModel.Street"></v-text-field>
                                                        </v-col>
                                                    </v-row>
                                                    <v-row>
                                                        <v-col cols="6" sm="6" md="6">
                                                            <v-text-field label="Ciudad" :rules="[rules.required]" v-model="bpModel.City"></v-text-field>
                                                        </v-col>
                                                        <v-col cols="6" sm="6" md="6">
                                                            <v-text-field label="Sector" :rules="[rules.required]" v-model="bpModel.Block"></v-text-field>
                                                        </v-col>
                                                    </v-row>
                                                </v-container>
                                            ...
                                            <v-card-actions>
                                                ...
                                                <v-btn color="blue darken-1" text @click="addBP">Agregar</v-btn>
                                            </v-card-actions>
                                        </v-card>
                                    </v-form>
                                </v-dialog>
                            ...
                            <v-flex xs12 md3>
                                <v-menu v-model="menuDocDate" :close-on-content-click="false" :nudge-right="40" transition="scale-transition" offset-y min-width="290px">
                                    <template v-slot:activator="{ on }">
                                        <v-text-field v-model="quotationModel.DocDate" label="Fecha de documento" prepend-icon="event" readonly :rules="[rules.required]" v-on="on"></v-text-field>
                                    </template>
                                    <v-date-picker v-model="quotationModel.DocDate" @input="menuDocDate = false"></v-date-picker>
                                </v-menu>
                            </v-flex>
                            <v-flex xs12 md12>
                                <v-data-table :headers="headers" :search="search" :items="quotationModel.DocumentLines" class="elevation-1">
                                    <template v-slot:top>
                                        <v-toolbar flat color="white">
                                            <v-toolbar-title>Cotización</v-toolbar-title>
                                            <v-divider class="mx-4" inset vertical></v-divider>
                                            <v-spacer></v-spacer>
                                            <v-text-field v-if="!disabled" class="text-xs-center" v-model="search" append-icon="search" label="Búsqueda" single-line hide-details></v-text-field>
                                            <v-spacer></v-spacer>
                                            <v-dialog v-model="dialog" persistent max-width="600px">
                                                <template v-slot:activator="{ on }">
                                                    <v-btn color="green" dark v-on="on" v-if="!disabled">
                                                        <v-icon left dark>mdi-package-variant-closed-plus</v-icon>
                                                        {{ formTitle }}
                                                    </v-btn>
                                                </template>
                                                            ...
                                                            <v-container>
                                                                <v-row justify="end" align="center">
                                                                    <v-col cols="8" sm="5" md="8">
                                                                        <v-autocomplete v-model="ItemCode" :items="items" :rules="[rules.required]" color="blue-grey lighten-2" label="Artículo" item-text="displayAutoComplete" item-value="ItemCode" hint="Código - Artículo" :loading="loadingItems"></v-autocomplete>
                                                                    </v-col>
                                              ...
                                              " @loaded="() => onLoaded()"></StreamBarcodeReader>
                                                                ...
                                                                <v-row>
                                                                    <v-col cols="5" sm="5" md="5">
                                                                        <v-text-field label="Precio" prefix="RD$" type="number" :rules="[
                                          rules.required,
                                          rules.positivevalue,
                                        ]" v-model="quotationDetailModel.UnitPrice"></v-text-field>
                                                                    </v-col>
                                                                    <v-col cols="5" sm="5" md="5">
                                                                        <v-text-field label="Cantidad" type="number" :rules="[
                                          rules.required,
                                          rules.positivevalue,
                                        ]" v-model="quotationDetailModel.Quantity"></v-text-field>
                                                                    </v-col>
                                        ...
                                        "></v-text-field>
                                                        ...
                                                        <v-card-actions>
                                                            <v-spacer></v-spacer>
                                                            <v-btn color="red darken-1" text @click="closeDialog()">Cerrar</v-btn>
                                                            <v-btn color="blue darken-1" text @click="addItem">Agregar</v-btn>
                                                        </v-card-actions>
                                                    </v-card>
                                                </v-form>
                                            </v-dialog>
                            ...
                            <v-container>
                                <v-row no-gutters>
                                    <v-col cols="12" sm="6">
                                        <v-container fluid>
                                            <v-textarea counter solo label="Comentarios" v-model="quotationModel.Comments"></v-textarea>
                                        </v-container>
                                    </v-col>
                                    <v-col cols="12" sm="2">
                                    ...
                                    <v-col cols="12" sm="4">
                                        <v-card class="pa-2" outlined tile>
                                            <v-text-field disabled prefix="RD$" label="Subtotal" v-model="quotationModel.SubTotal"></v-text-field>
                                            <v-text-field disabled prefix="RD$" label="Impuestos" v-model="quotationModel.Tax"></v-text-field>
                                            <v-text-field disabled prefix="RD$" label="Total" v-model="quotationModel.Total"></v-text-field>
                                        </v-card>
                                    </v-col>
                                </v-row>
                            </v-container>
                            ...
                            <v-flex xs12 text-xs-right>
                                ...
                                <v-btn class="mx-0 font-weight-light" color="success" @click="save" v-if="!disabled">
        ...
        async getBusinessPartners() {
            ...
            querySnapshot.forEach((doc) => {
                this.businessPartners.push({
                    ...
                    displayAutoComplete: doc.data().CardCode +
                        " - " +
                        doc.data().CardName +
                        " - " +
        ...
        async getItems() {
            ...
            querySnapshot.forEach((doc) => {
                this.items.push({
                    ...
                    displayAutoComplete: doc.data().ItemCode + " - " + doc.data().ItemName,
                });
            });
